...
const Home = () => {
  ...
  async function deleteClothing(id) {
    try {
      await remove(
        ref(db, "users/" + auth.currentUser.uid + "/clothings/" + id),
      );
    } catch (error) {
      alert(error.message);
      console.error("Error:", error.message);
  ...
  return (
    <div className="clothing-form-container">
      <form onSubmit={handleNewClothing} className="clothing-form">
        ...
        <button id="create-clothing" type="submit">
          Create Clothing
        </button>
      </form>
      ...
      <div className="clothings-display">
        ...
        <div className="clothing-list">
          {userData && userData.clothings ? (
            Object.keys(userData.clothings).map((key) => (
              <div id={key} className="clothing-box">
                <strong>{userData.clothings[key].name}</strong> -{" "}
                {userData.clothings[key].category}
                <br />
                <p>
                  Tags:{" "}
                  {userData.clothings[key].tags
                    ? userData.clothings[key].tags.join(", ")
                    : "No tags"}
                </p>
                <br />
                <img
                ...
                <br />
                <button
                  onClick={() => deleteClothing(key)}
                  className="delete-clothing"
                >
                  Delete
                </button>
              </div>
